...
namespace dtsw{
  ...
  void mult(Data *t,Data *d, Data *h, SWTask *p){
    ...
    for(int i=0; i< _D.size(); i++){
      int row = _D(i).sp_row;
      int col = _D(i).sp_col;
      DiffTask *Diff = new DiffTask(_T(row),_D(row,col),_H(col),p);
  ...
  void rhs(Data *dH , Data *t,Data *h, SWTask *p){
    ...
    for(int i=0; i< nb; i++){
      RHSTask *RHS= new RHSTask(DH(i),_T(i),_H(i),p);
  ...
  void add (Data *a1,Data *a2, double dt, Data *a4, SWTask *p){
    ...
    int nb = Parameters.nb;
    for(int i=0; i< nb; i++){
      AddTask *Add = new AddTask(Y(i),A(i),dt,X(i),p);
  ...
  void step(Data *h, Data *f1, Data *f2 , Data *f3 , Data *f4, SWTask *p){
    ...
    int nb = Parameters.nb;
    for(int i=0; i< nb; i++){
      StepTask * Step=new StepTask(_H(i),_F1(i),_F2(i),_F3(i),_F4(i),p);
